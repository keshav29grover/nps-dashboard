...
export default function Header({ lastUpdated, onRefresh, loading, theme, onToggleTheme }) {
  ...
  const parsedUpdated = lastUpdated ? dayjs(lastUpdated) : null
  const updatedLabel = parsedUpdated
    ? (parsedUpdated.hour() === 0 && parsedUpdated.minute() === 0
        ? parsedUpdated.format('DD MMM YYYY')
        : parsedUpdated.format('DD MMM HH:mm'))
    : null
